...
@interface MCCustomAlertView ()<MCShadowViewDelegate> {
    CGFloat _layoutOrigin;
    UIWindow *_window;
    BOOL _hasFirstResponsderContent;
    //    MCCustomAlertView __strong *_retainedSelf;
}
...
@implementation MCCustomAlertView
...
- (UIButton *)buttonWithTitle:(NSString *)title image:(UIImage *)image color:(UIColor *)color {
...
- (UIButton *)cancelButton:(NSString *)title {
    return [self buttonWithTitle:title image:nil color:[UIColor colorWithHEX:0x8c8c8c]];
}

- (UIButton *)submitButton:(NSString *)title {
    return [self buttonWithTitle:title image:nil color:[UIColor navigationBarColor]];
}
...
+ (MCCustomAlertView *)viewWithTitle:(NSString *)title customView:(UIView *)customView cancelButtonTitle:(NSString *)cancelButtonTitle otherButtonTitles:(NSString *)otherButtonTitles, ... {
    va_list args;
//    if (otherButtonTitles) {
        va_start(args, otherButtonTitles);
//    }
    MCCustomAlertView *alertView = [[self alloc] initWithCustomView:customView title:title cancelButtonTitle:cancelButtonTitle otherButton:otherButtonTitles list:args];
//    if (otherButtonTitles) {
        va_end(args);
//    }
    return alertView;
...
+ (MCCustomAlertView *)viewWithTitle:(NSString *)title message:(NSString *)message cancelButtonTitle:(NSString *)cancelButtonTitle otherButtonTitles:(NSString *)otherButtonTitles, ... {
    UIFont * font = [UIFont systemFontOfSize:15];
    NSAttributedString * attribute = [[NSAttributedString alloc] initWithString:message attributes:@{NSFontAttributeName:font}];
    CGSize size = [attribute boundingRectWithSize:CGSizeMake(250, NSIntegerMax) options:NSStringDrawingUsesLineFragmentOrigin context:nil].size;
...
//    if (otherButtonTitles) {
        va_start(args, otherButtonTitles);
//    }
    MCCustomAlertView *view = [[self alloc] initWithCustomView:customView title:title cancelButtonTitle:cancelButtonTitle otherButton:otherButtonTitles list:args];
//    if (otherButtonTitles) {
        va_end(args);
//    }
    return view;
    ...
    if (removeStack) {
        if ([alert_queue lastObject] == self) {
            ...
            if (alert_queue.count) {
                [(MCCustomAlertView *)[alert_queue lastObject] showWithFirstTime:NO];
            } else {
                alert_queue = nil;
            }
        }
...
- (MCCustomAlertView *)alertWithTitle:(NSString *)title cancelButtonTitle:(NSString *)cancelButtonTitle otherButtonTitles:(NSString *)otherButtonTitles, ... {
    va_list args;
//    if (otherButtonTitles != nil) {
        va_start(args, otherButtonTitles);
//    }
    MCCustomAlertView *alertView = [[MCCustomAlertView alloc] initWithCustomView:self title:title cancelButtonTitle:cancelButtonTitle otherButton:otherButtonTitles list:args];
//    if (otherButtonTitles) {
        va_end(args);
//    }
